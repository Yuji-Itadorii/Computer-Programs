// https://practice.geeksforgeeks.org/problems/k-centers-problem/1
...
{
public:
    int solve(int n, int k, vector<vector<int>> &mat, int maxx)
    {
    ...
    int selectKcities(int n, int k, vector<vector<int>> &mat)
    {
        ...
        {
            maxx = i;
            ans = min(ans, solve(n, k, mat, maxx));
        }
